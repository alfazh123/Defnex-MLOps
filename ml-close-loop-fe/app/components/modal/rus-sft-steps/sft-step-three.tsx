										...
										<div
											...
											onClick={() => {
												if (!provider.disabled) {
													...
													setSelectedComputeRes(
														provider.computeRes[0],
													);
												}
											}}>
						...
						{selectedProvider?.id === "google-colab" && (
							<div className="flex flex-col gap-2 p-4 rounded-lg border border-amber-300 bg-amber-50/40">
								...
								<span className="text-xs text-amber-700/80 pt-1">
									Configured centrally in Infrastructure →
									Colab. Opens notebook template in a new tab.
								</span>
							</div>
						)}
									...
									(res, index) => (
										<div
											...
											<div className="flex items-center gap-2">
												{res.isAuto && (
													<LightningIcon className="h-4 w-4 text-blue-600 shrink-0" />
												)}
