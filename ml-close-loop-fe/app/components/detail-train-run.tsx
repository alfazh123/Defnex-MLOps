...
import { ArrowRightIcon, CopyIcon } from "@phosphor-icons/react/dist/ssr";
		...
		<div
			...
			<div className="flex flex-wrap justify-between items-center font-semibold text-sm mb-2">
				<div>
					...
					<p className="text-xs text-accent-foreground font-light">
						{run.runId}
					</p>
				</div>
				<div>
			...
			<div className="flex flex-col space-y-4 mt-4">
					...
					<div className="grid grid-cols-2 bg-accent border border-gray-200 rounded-md p-2 text-sm">
						<div className="flex flex-col gap-2">
							...
							<p>
								Epochs:{" "}
								<span className="font-bold">
									{run.loraConfig.epochs}
								</span>
							</p>
							<p>
								Max Seq:{" "}
								<span className="font-bold">
									{run.loraConfig.maxSeqLength} tokens
								</span>
							</p>
						</div>
						<div className="flex flex-col gap-2">
							<p>
								Batch:{" "}
								<span className="font-bold">
									{run.loraConfig.batch}
								</span>
							</p>
							<p>
								LoRA r/α:{" "}
								<span className="font-bold">
									{run.loraConfig.loraRank}/
									{run.loraConfig.loraAlpha}
								</span>
							</p>
						</div>
					</div>
				...
				<div className="flex flex-col gap-2">
					...
					<div
						id={`checkppoint-${run.runId}`}
						className="relative bg-accent border border-gray-200 rounded-md p-2 text-sm break-all">
						<div className="absolute top-2 right-2">
							<CopyIcon
								className="h-4 w-4 text-accent-foreground cursor-pointer"
								onClick={() => {
									...
									toast.add({
										...
										description:
											"Checkpoint path copied to clipboard",
										type: "success",
									});
								}}
							/>
				...
				<div className="flex justify-center items-center w-full">
					<Button
						...
						View Registered Model in Registry <ArrowRightIcon />
					</Button>
				</div>
			</div>
		</div>
