                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <Input
                                ...
                                onChange={(e) => onChange({ goldenSet: e.target.value })}
                            />
                        </div>
                        ...
                        <div className="flex flex-col gap-3 border p-4 rounded-lg bg-blue-50/10 border-blue-100">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <Field>
                                    ...
                                    <Input value={value.qaGate.structuralValidity} onChange={(e) => onChange({ qaGate: { ...value.qaGate, structuralValidity: Number(e.target.value) } })} />
                                </Field>
                                <Field>
                                    <FieldLabel className="text-xs text-muted-foreground">
                                        Domain Quality ≥
                                    </FieldLabel>
                                    <Input value={value.qaGate.domainQuality} onChange={(e) => onChange({ qaGate: { ...value.qaGate, domainQuality: Number(e.target.value) } })} />
                                </Field>
                            </div>
                            <div className="flex flex-col gap-1 pt-2 border-t text-xs">
                                <span className="text-muted-foreground">
                                    Regression score: <span className="font-semibold text-blue-900">&gt;= previous champion</span>
                                </span>
                                <span className="text-muted-foreground">
                                    Model promotion requires evaluation to pass the configured quality gate.
                                </span>
                            </div>
                        </div>
                    </div>
