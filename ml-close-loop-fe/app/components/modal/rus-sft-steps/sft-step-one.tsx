                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <Field>
                                ...
                                <Select value={value.dataset} onValueChange={(val) => onChange({ dataset: val ?? "", datasetVersion: datasetForSft.find((data) => data.value === val)?.versions[0]?.value ?? "" })} items={datasetForSft}>
                                    <SelectTrigger>
                            ...
                            <Field>
                                ...
                                <Select value={value.datasetVersion} onValueChange={(val) => onChange({ datasetVersion: val ?? "" })} items={datasetForSft.find((data) => data.value === value.dataset)?.versions}>
                                    <SelectTrigger>
                                        <SelectValue placeholder="Select version" />
                                    </SelectTrigger>
                                    <SelectContent>
                                        {datasetForSft.find((data) => data.value === value.dataset)?.versions.map((version) => (
                        ...
                        <div className="flex flex-col gap-2 border p-3 rounded-lg bg-white">
                            <div className="grid grid-cols-3 gap-2">
                                ...
                                <div className="p-3 rounded-md border bg-white">
                                    ...
                                    <span className="font-bold text-sm">
                                        {datasetForSft.find((data) => data.value === value.dataset)?.sampleCount ?? "-"}
                                    </span>
                                </div>
                            ...
                            <div className="flex justify-between items-center pt-2 px-1 text-xs text-muted-foreground">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span>
                                        Immutable — SHA-256
                                        {datasetForSft.find((data) => data.value === value.dataset)?.shaImmutable ?? "6d2258a41920da78..."}
                                    </span>
                                </div>
